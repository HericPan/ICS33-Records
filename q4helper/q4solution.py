...
#     iterable = list(i for i in iterable)
#     n = len(iterable) - n
#     i = iter(iterable)
#     c = 1
#     while True:
...
#         yield next(i)
    i = iter(iterable)
    try:
        l = list(next(i) for _ in range(n))
        while True:
            current = next(i)
            yield l.pop(0)
            l.append(current)
...
def alternate_all(*args):
    iters = list(iter(iterable) for iterable in args)
    
    while any(i!=None for i in iters):
...
def min_key_order(adict):
    if adict == {}:
        return
    pk = None
    while pk == None or pk < min(k for k in adict.keys() if k > pk):
        pk = min(k for k in adict.keys() if k > pk) if pk != None else min(k for k in adict.keys())
        yield (pk, adict[pk])
